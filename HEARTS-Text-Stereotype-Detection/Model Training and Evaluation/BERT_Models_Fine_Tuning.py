...
def train_model(train_data, model_path, batch_size, epoch, learning_rate, model_output_base_dir, dataset_name, seed):
    ...
    model_output_dir = str(model_output_dir)  # Convert back to string for Trainer compatibility
    ...
    training_args = TrainingArguments(
        ...
        use_cpu=False,  # Ensure GPU is enabled (no_cuda is deprecated)
...
# Helper function to get Windows long path (bypass 260 char limit)
def get_long_path(path_str):
    """Convert path to Windows long path format if needed"""
    abs_path = os.path.abspath(path_str)
    if os.name == 'nt' and len(abs_path) > 200:  # Windows with long path
...
def evaluate_model(test_data, model_output_dir, result_output_base_dir, dataset_name, seed):
    ...
    print(f"Output directory created: {result_output_dir}")

    # Use GPU (device=0) if available, otherwise CPU (device=-1)
    import torch
    ...
    # Save full results - use Windows long path prefix to bypass 260 char limit
    results_file_path = get_long_path(str(result_output_dir / "full_results.csv"))
    results_df.to_csv(results_file_path, index=False)
    print(f"Saved: {results_file_path}")

    # Save classification report - use Windows long path prefix
    report = classification_report(y_true, pred_labels, output_dict=True)
    df_report = pd.DataFrame(report).transpose()
    result_file_path = get_long_path(str(result_output_dir / "classification_report.csv"))
...
# ============================================================
# ALBERT-V2 model - ALREADY COMPLETED, COMMENTED OUT
# ============================================================
