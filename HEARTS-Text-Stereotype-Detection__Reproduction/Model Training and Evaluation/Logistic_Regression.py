...
def train_model(train_data, model_output_base_dir, dataset_name, feature_type, seed):
    ...
    model_output_base_dir = os.path.normpath(model_output_base_dir)
...
def evaluate_model(test_data, model_output_dir, result_output_base_dir, dataset_name, feature_type, seed):
    ...
    result_output_base_dir = os.path.normpath(result_output_base_dir)
    ...
    print(f"Created/verified directory: {result_output_dir}")
    ...
    # Use Windows long path prefix to bypass 260 char limit
    results_file_path = get_long_path(os.path.join(result_output_dir, "full_results.csv"))
    results_df.to_csv(results_file_path, index=False)
    print(f"Saved: {results_file_path}")

    report = classification_report(y_true, pred_labels, output_dict=True)
    df_report = pd.DataFrame(report).transpose()
    result_file_path = get_long_path(os.path.join(result_output_dir, "classification_report.csv"))
...
# COMMENTED OUT - Already completed, no need to rerun
